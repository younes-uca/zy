...
const AchatItemAdminList: React.FC = () =>  {
    ...
    const handleFetchAndUpdate = async (id: number) => {
        try {
            ...
            navigation.navigate('AchatItemAdminUpdate', { achatItem: achatItemData });
        } catch (error) {
    ...
    const handleFetchAndDetails = async (id: number) => {
        try {
            const achatItemResponse = await service.find(id);
            const achatItemData = achatItemResponse.data;
            navigation.navigate('AchatItemAdminDetails', { achatItem: achatItemData });
        } catch (error) {
            console.error('Error fetching achat item data:', error);
        }
    };
...
return(
    <ScrollView showsVerticalScrollIndicator={false} style={globalStyle.scrollViewList}>
        ...
        <Text style={globalStyle.textHeaderList} >Achat item List</Text>
        ...
        <View style={{ marginBottom: 100 }}>
            {achatItems && achatItems.length > 0 ? ( achatItems.map((achatItem) => (
                ...
                <Text style={globalStyle.textNotFound}>No achat items found.</Text>
            )}
        </View>

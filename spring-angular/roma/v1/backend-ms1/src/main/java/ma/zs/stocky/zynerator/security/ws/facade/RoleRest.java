...
public class RoleRest  extends AbstractController<Role, RoleDto, RoleCriteria, RoleService, RoleConverter> {
    ...
    @GetMapping("id/{id}")
    public ResponseEntity<RoleDto> findById(@PathVariable Long id, String[] includes, String[] excludes) throws Exception {
        return super.findById(id, includes, excludes);
    }

    @Operation(summary = "Finds a role by authority")
    @GetMapping("authority/{authority}")
    public ResponseEntity<RoleDto> findByAuthority(@PathVariable String authority, String[] includes, String[] excludes) throws Exception {
        return super.findByReferenceEntity(new Role(authority), includes, excludes);
    }

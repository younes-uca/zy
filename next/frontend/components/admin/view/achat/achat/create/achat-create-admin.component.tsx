...
const Create: React.FC<AchatCreateAdminType> = ({visible, onClose, add, showToast, list, service, t}) => {
    ...
    const onInputDateChangeAchatItems = (e: any, name: string) => {
        const val = e.value || '';
            ...
            <TabPanel header={t("achat.achatItems")}>
                <TabView activeIndex={activeTab} onTabChange={(e) => setActiveTab(e.index)}>
                    <TabPanel header={t("creation")}>
                        <div className="grid">
                            <div className="field col-6">
                                ...
                                <Dropdown id="produitDropdown" value={achatItems.produit} options={produits} onChange={(e) => onDropdownChangeAchatItems(e, 'produit')}    placeholder={t("achatItem.produitPlaceHolder")} filter filterPlaceholder={t("achatItem.produitPlaceHolderFilter")}  optionLabel="reference" showClear />
                             </div>
                            <div className="field col-6">
                    ...
                    <DataTable value={item.achatItems} tableStyle={{minWidth: '50rem'}} dataKey="id">
                        ...
                        <Button icon="pi pi-pencil" rounded severity="success" className="mr-2" onClick={()=> editAchatItems(rowData)} /> </div>)}></Column>
                    </DataTable>
                    </div>
                    </TabPanel>
                </TabView>

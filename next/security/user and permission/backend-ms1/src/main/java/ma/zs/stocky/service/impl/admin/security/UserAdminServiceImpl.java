...
public class UserAdminServiceImpl extends AbstractServiceImpl<User, UserCriteria, UserDao> implements UserAdminService {
    ...
    public User create(User t) {
        ...
        if (saved != null && t.getModelPermissionUsers() != null) {
                t.getModelPermissionUsers().forEach(element-> {
                    element.setUser(saved);
                    modelPermissionUserService.create(element);
            });
        }
        return saved;
    ...
    @Autowired
    private ModelPermissionUserAdminService modelPermissionUserService ;
    @Autowired

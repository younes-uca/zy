...
type ActionPermissionEditAdminType = {
    visible: boolean,
    onClose: () => void,
    showToast: React.Ref<Toast>,
    selectedItem: ActionPermissionDto
    update: (item: ActionPermissionDto) => void,
    list: ActionPermissionDto[],
    service: ActionPermissionAdminService,
    t: TFunction
}
const Edit: React.FC<ActionPermissionEditAdminType> = ({visible, onClose, showToast, selectedItem, update, list, service, t}) => {
    ...
    const {
        ...
        parseToIsoFormat,
        adaptDate
        } = useEditHook<ActionPermissionDto, ActionPermissionCriteria>({list, selectedItem, onClose, update, showToast,service, t, isFormValid})

...
import Edit from '@/components/admin/view/security/model-permission/edit/model-permission-edit-admin.component';
import Create from '@/components/admin/view/security/model-permission/create/model-permission-create-admin.component';
import View from '@/components/admin/view/security/model-permission/view/model-permission-view-admin.component';
...
const List = () => {
    ...
    const emptyCriteria = new ModelPermissionCriteria();
    const service = new ModelPermissionAdminService();
    ...
    const {
        ...
        filter
    } = useListHook<ModelPermissionDto, ModelPermissionCriteria>({ emptyItem, emptyCriteria,service, t})
    ...
    <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
        <h5 className="m-0">{t("modelPermission.header")}</h5>
        ...
        <div className="col-12">
            <div className="card">
                ...
                <Card title={t("search")} className="mb-5">
                        <div className="grid">
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="1">{t("modelPermission.reference")}</label>
                                <InputText id="1" value={criteria.reference} onChange={(e) => setCriteria({ ...criteria, reference: e.target.value })} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="2">{t("modelPermission.libelle")}</label>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems as any} onSelectionChange={(e) => setSelectedItems(e.value as any)} dataKey="id" className="datatable-responsive" filters={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="reference" header={t("modelPermission.reference")} sortable></Column>
                    
                    
                    <Column field="libelle" header={t("modelPermission.libelle")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("modelPermission.deleteModelPermissionConfirmationMessage")} <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
            ...
            <Dialog visible={deleteItemsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                    {item && <span>{t("modelPermission.deleteModelPermissionsConfirmationMessage")}</span>}
                </div>
            </Dialog>

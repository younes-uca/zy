...
import Edit from '@/components/admin/view/security/role/edit/role-edit-admin.component';
import Create from '@/components/admin/view/security/role/create/role-create-admin.component';
import View from '@/components/admin/view/security/role/view/role-view-admin.component';
...
const List = () => {
    ...
    const emptyCriteria = new RoleCriteria();
    ...
    const {
        ...
        filter
    } = useListHook<RoleDto, RoleCriteria>({ emptyItem, emptyCriteria,service, t})
    ...
    <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
        <h5 className="m-0">{t("role.header")}</h5>
        ...
        <div className="col-12">
            <div className="card">
                ...
                <Card title={t("search")} className="mb-5">
                        <div className="grid">
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="1">{t("role.authority")}</label>
                                <InputText id="1" value={criteria.authority} onChange={(e) => setCriteria({ ...criteria, authority: e.target.value })} />
                            </div>
                        </div>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems as any} onSelectionChange={(e) => setSelectedItems(e.value as any)} dataKey="id" className="datatable-responsive" filters={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="authority" header={t("role.authority")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("role.deleteRoleConfirmationMessage")} <b>{item.authority}</b>?</span>)}
                    </div>
                </Dialog>
            ...
            <Dialog visible={deleteItemsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                    {item && <span>{t("role.deleteRolesConfirmationMessage")}</span>}
                </div>
            </Dialog>
